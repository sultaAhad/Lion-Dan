<link rel="stylesheet" href="css/style.css">
<link rel="stylesheet" href="css/variable.css">

<!-- App Include Here -->
<?php include 'inc/app.php'; ?>
<!-- App Include Here -->

<!-- Body Content Start Here -->
<section class="slider-sec">
    <div class="slider-wrapper">
        <div class="Modern-Slider">
            <!-- Item -->
            <div class="item">
                <div class="img-fill">
                    <img src="img/banner1.png" class="img-fluid" alt="">
                    <div class="info">
                        <div>
                            <h5><img src="img/bannerLogo.png" class="img-fluid" alt=""></h5>
                            <h3>The Unique Barber <br> Experience</h3>
                            <a class="btn" href="">BOOK APPOINTMENT</a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- // Item -->
            <!-- Item -->
            <div class="item">
                <div class="img-fill">
                    <img src="img/banner2.png" class="img-fluid" alt="">
                    <div class="info">
                        <div>
                            <h5><img src="img/bannerLogo.png" class="img-fluid" alt=""></h5>
                            <h3>The Unique Barber <br> Experience</h3>
                            <a class="btn" href="">BOOK APPOINTMENT</a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- // Item -->
            <!-- Item -->
            <div class="item">
                <div class="img-fill">
                    <img src="img/banner1.png" class="img-fluid" alt="">
                    <div class="info">
                        <div>
                            <h5><img src="img/bannerLogo.png" class="img-fluid" alt=""></h5>
                            <h3>The Unique Barber <br> Experience</h3>
                            <a class="btn" href="">BOOK APPOINTMENT</a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- // Item -->
            <!-- Item -->
            <div class="item">
                <div class="img-fill">
                    <img src="img/banner2.png" class="img-fluid" alt="">
                    <div class="info">
                        <div>
                            <h5><img src="img/bannerLogo.png" class="img-fluid" alt=""></h5>
                            <h3>The Unique Barber <br> Experience</h3>
                            <a class="btn" href="">BOOK APPOINTMENT</a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- // Item -->
        </div>
    </div>
</section>
<!-- Body Content End Here -->
<!-- category-sec start here -->
<section class="step-back">
    <div class="background-img">
        <img src="img/background3.png" class="img-fluid" alt="">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-4">
                <div class="img-step">
                    <img src="img/lion4.png" class="img-fluid" alt="">
                </div>
            </div>
            <div class="col-lg-6 align-items-center">
                <div class="step-heading">
                    <div class="row">
                        <div class="col-lg-12">
                            <img src="img/step-img.png" class="img-fluid" alt="">
                            <h2>step back in time and elevate your grooming experience</h2>
                            <p> At Lions Den Barber Shop, our traditional barber chairs, hot lather shaves, and head and neck massages set us apart in the barbering industry. Whether you're looking for a classic buzz cut or a modern men's hairstyle, our skilled barbers will exceed your expectations. </p>
                            <a class="btn service" href="">more about us</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- category-sec end here -->
<section class="our-services">
    <div class="our-background">
        <img src="img/background1.png" class="img-fluid" alt="">
    </div>
    <div class="our-background1">
        <img src="img/background2.png" class="img-fluid" alt="">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-7 mx-auto text-center">
                <div class="img-our-service">
                    <img src="img/footer-logo.png" class="img-fluid" alt="">
                    <h4>Browse our service</h4>
                    <p> Lions Den is an exclusive barbershop located in
                        Cliffside Park, New Jersey & Hoboken, </p>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="mans-wrapper">
                            <h5>mens haircut</h5>
                            <h6>$250.00</h6>
                            <p> Lions Den is an exclusive barbershop located in
                                Cliffside Park, New Jersey & Hoboken,</p>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="mans-wrapper golden-border">
                            <h6 class="fovirate">member foverite!</h6>
                            <h5> haircut and beard line up</h5>
                            <h6>$50.00</h6>
                            <p> Lions Den is an exclusive barbershop located in
                                Cliffside Park, New Jersey & Hoboken,</p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-6">
                        <div class="mans-wrapper">
                            <h5> Haircut,Beard, Line Up,Hot Towel & Face Massage
                            </h5>
                            <h6>$60.00</h6>
                            <p> This service includes Lions Den Haircut of your choice with a beard line up using our hot lather, our signature shave gel, hot towel and finished with a luxurious menthol massage.</p>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="mans-wrapper ">
                            <h5>lions den experience</h5>
                            <h6>$125.00</h6>
                            <p> This service consists of a Haircut of choice with the beard shave or line up. It includes hot towel,face massage,exfoliting cream. also a steamer and a wash. With neck massage and styling.</p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12 mt-5 mb-5 text-center">
                        <div class="our-service-btn1">
                            <a class="btn service" href="">book appointment</a>
                            <a class="btn service1" href="">browse all service</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="we-offer">
    <div class="our-backgroun3">
        <img src="img/background4.png" class="img-fluid" alt="">
    </div>
    <div class="our-background4">
        <img src="img/background5.png" class="img-fluid" alt="">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="our-offer-heading">
                    <div class="row pb-5 align-items-center">
                        <div class="col-lg-9">
                            <div class="heading-offer">
                                <h2>WE OFFER AN UNIQUE EXPERIENCE</h2>
                                <p> A barber experience that transports you back in time to the 1920s, complete with vintage décor, old-school barbering techniques, and a complimentary glass of whiskey. </p>
                            </div>
                        </div>
                        <div class="col-lg-3">
                            <a class="btn service" href="">book appointment</a>
                        </div>
                    </div>
                    <div class="row mt-5">
                        <div class="col-lg-4">
                            <img src="img/lion6.png" class="img-fluid" alt="">
                        </div>
                        <div class="col-lg-4">
                            <div class="book-img-wrapper">
                                <img src="img/lion13.png" class="img-fluid" alt="">
                                <img src="img/lion10.png" class="img-fluid" alt="">
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <img src="img/lion7.png" class="img-fluid" alt="">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="take-look">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto text-center">
                <div class="img-our-service12 workportfolio">
                    <div class="icon-service-wrapper ">
                        <i class="fa fa-play" aria-hidden="true"></i>
                    </div>
                    <h4>TAKE A LOOK AT OUR WORK</h4>
                    <p> A portfolio is not just a collection of your work. It's a reflection of who you are as a professional..
                    </p>
                    <div class="bookbtn-service">
                        <a class="btn service12" href="">BOOK APPOINTMENT</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="why-choose">
    <div class="our-backgroun7">
        <img src="img/background6.png" class="img-fluid" alt="">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-5">
                <div class="why-choose-us-heading">
                    <h3>WHY CHOOSE US</h3>
                    <p> At our barbershop, we believe that grooming is an essential part of every man's routine. That's why we offer a variety of services designed to cater to your unique needs, whether you're looking for a classic cut, a modern style, or a relaxing shave. </p>
                </div>
                <div class="top-bubbles">
                    <ul>
                        <li><img src="img/hairstyle.png" class="img-fluid" alt="">
                            <div class="top-banner">
                                <h5>top banners</h5>
                                <p> Our team of expert barbers takes great pride in their craft, and we use only Lions Den products to ensure that you leave our barbershop looking and feeling your best.
                                </p>
                            </div>
                        </li>
                        <li><img src="img/barber-chair.png" class="img-fluid" alt="">
                            <div class="top-banner">
                                <h5>premium services</h5>
                                <p> At our high-end barbershop, we believe that every client deserves exceptional service, and that's what we strive to deliver. From the moment you walk through our doors, you'll be greeted with a warm welcome and a commitment to excellence
                                </p>
                            </div>
                        </li>
                    </ul>
                    <div class="col-lg-6">
                        <a class="btn service" href="">book appointment</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-1 p-0"></div>
            <div class="col-lg-6">
                <div class="dbl-img">
                    <div class="premium-wrapper-img">
                        <img src="img/lion15.png" class="img-fluid" alt="">
                    </div>
                    <div class="premium-wrapper-img1">
                        <img src="img/lion18.png" class="img-fluid" alt="">
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="our-services">
    <div class="our-backgroun10">
        <img src="img/background7.png" class="img-fluid" alt="">
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-7 mx-auto text-center">
                <div class="img-our-service">
                    <img src="img/footer-logo.png" class="img-fluid" alt="">
                    <h4>Browse our products</h4>
                    <p> Products that truly resonate with customers and solve real problems and make their lives better </p>
                </div>
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-1"></div>
            <div class="col-lg-11 mt-5">
                <div class="content">
                    <div class="slider-gell">
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell1.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell2.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell3.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell4.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell4.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell4.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                        <div class="product-detail">
                            <div class="mastache">
                                <img src="img/gell4.png" class="img-fluid" alt="">
                            </div>
                            <h4>Bread & mustache care oil</h4>
                            <h5>$25.00</h5>
                        </div>
                    </div> <!-- end slider -->
                    </span>
                </div> <!-- end content -->
            </div>
        </div>
        <div class="row">
            <div class="col-lg-8 mt-4 mx-auto">
                <div class="progress" role="progressbar" aria-valuemin="0" aria-valuemax="100">
                    <span class="slider__label sr-only">
                </div>
            </div>
        </div>
    </div>
</section>
<section class="we-offer client slider">
    <div class="background-footer8">
        <img src="img/background8.png" class="img-fluid" alt="">
    </div>
    <div class="container-fluid p-0">
        <div class="row">
            <div class="col-lg-2"></div>
            <div class="col-lg-10 p-0">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="our-offer-heading">
                            <div class="row mb-3 align-items-center">
                                <div class="col-lg-9">
                                    <div class="heading-offer google-reviews">
                                        <img src="img/footer-logo.png" class="img-fluid" alt="">
                                        <h2>WHAT OUR CLIENTS SAY</h2>
                                        <p> With 450+ Google Reviews (and counting), we know that our customers are happy. See what they said about our professional barbers. </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="slider-wrapper1">
                    <div class="bread-man">
                        <div class="row">
                            <div class="col-lg-2 p-0">
                                <img src="img/bread.png" class="img-fluid" alt="">
                            </div>
                            <div class="col-lg-10">
                                <ul class="bread-wrapper-content">
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                </ul>
                                <h4>“THIS BARBER SHOP IS SIMPLY AWESOME”</h4>
                                <p> Et proin ut in dignissim sem non a nullam magna lectus urna et dui quam tellus imperdiet sit purus at fringilla scelerisque diam amet fermentum orci fringilla aliquet nulla lectus erat eu auctor diam potenti turpis interdum eu.</p>
                                <h4>JOHN CARTER <span>HOBOKEN</span> </h4>
                            </div>
                        </div>
                    </div>
                    <div class="bread-man">
                        <div class="row">
                            <div class="col-lg-2 p-0">
                                <img src="img/bread.png" class="img-fluid" alt="">
                            </div>
                            <div class="col-lg-10">
                                <ul class="bread-wrapper-content">
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                </ul>
                                <h4>“THIS BARBER SHOP IS SIMPLY AWESOME”</h4>
                                <p> Et proin ut in dignissim sem non a nullam magna lectus urna et dui quam tellus imperdiet sit purus at fringilla scelerisque diam amet fermentum orci fringilla aliquet nulla lectus erat eu auctor diam potenti turpis interdum eu.</p>
                                <h4>JOHN CARTER <span>HOBOKEN</span> </h4>
                            </div>
                        </div>
                    </div>
                    <div class="bread-man">
                        <div class="row">
                            <div class="col-lg-2 p-0">
                                <img src="img/bread.png" class="img-fluid" alt="">
                            </div>
                            <div class="col-lg-10">
                                <ul class="bread-wrapper-content">
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                </ul>
                                <h4>“THIS BARBER SHOP IS SIMPLY AWESOME”</h4>
                                <p> Et proin ut in dignissim sem non a nullam magna lectus urna et dui quam tellus imperdiet sit purus at fringilla scelerisque diam amet fermentum orci fringilla aliquet nulla lectus erat eu auctor diam potenti turpis interdum eu.</p>
                                <h4>JOHN CARTER <span>HOBOKEN</span> </h4>
                            </div>
                        </div>
                    </div>
                    <div class="bread-man">
                        <div class="row">
                            <div class="col-lg-2 p-0">
                                <img src="img/bread.png" class="img-fluid" alt="">
                            </div>
                            <div class="col-lg-10">
                                <ul class="bread-wrapper-content">
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                    <li><i class="fa fa-star" aria-hidden="true"></i>
                                    </li>
                                </ul>
                                <h4>“THIS BARBER SHOP IS SIMPLY AWESOME”</h4>
                                <p> Et proin ut in dignissim sem non a nullam magna lectus urna et dui quam tellus imperdiet sit purus at fringilla scelerisque diam amet fermentum orci fringilla aliquet nulla lectus erat eu auctor diam potenti turpis interdum eu.</p>
                                <h4>JOHN CARTER <span>HOBOKEN</span> </h4>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12 mt-5 mb-5 text-center">
                <div class="our-service-btn">
                    <a class="btn service" href="">book appointment</a>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- Footer Include Here -->
<?php include 'inc/footer.php'; ?>
<!-- Footer Include Here -->